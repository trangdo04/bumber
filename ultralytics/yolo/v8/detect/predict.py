...
def ocr_image(img,coordinates):
    ...
    print("text :", str(text)) 
    return str(text) # return the number plate text in each frame
...
class DetectionPredictor(BasePredictor):
    ...
    def write_results(self, idx, preds, batch, is_image=False):
        p, im, im0 = batch
        log_string = ""
        if len(im.shape) == 3:
        ...
        # Nếu là ảnh, trả về text OCR
        if is_image and text_results:
            print(f"Detected texts: {', '.join(text_results)}")
