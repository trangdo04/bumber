...
class BasePredictor:
    ...
    def __call__(self, source=None, model=None):
        ...
        is_image = True
        # if source is None:
        #     raise ValueError("Source cannot be None, please provide a valid file path.")
        # # Loại bỏ dấu ngoặc kép ở đầu và cuối chuỗi nếu có
        # source = source.strip('"')

        # # Kiểm tra loại tệp input là ảnh hay video
        # is_image = source.lower().endswith(('.jpg', '.jpeg', '.png'))
        ...
        for batch in self.dataset:
            ...
            for i in range(len(im)):
                ...
                p = Path(path)
                if is_image:  # Nếu input là ảnh
                    result_text = self.write_results(i, preds, (p, im, im0s), is_image=True)  # Chỉ trả về text
                    if result_text:  # Nếu có kết quả OCR
                        print(f"Detected text from image: {result_text}")
                        return result_text  # Trả về text của biển số xe
                else:  # Nếu input là video
                    s += self.write_results(i, preds, (p, im, im0s), is_image=False)
